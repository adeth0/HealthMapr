// ─────────────────────────────────────────────────────────────────────────────
// Insight Rule — Fatigue Risk (High activity + Low calories + Low sleep)
// ─────────────────────────────────────────────────────────────────────────────
...
export function evaluateFatigue(
  ...
  // High-load combo: low sleep + significant calorie deficit + active
  ...
  if (riskCount >= 2) {
    ...
    return {
      ...
      message: `You're combining ${factors.join(", ")}. This combination will accelerate fatigue, impair recovery, and risk overtraining. Your body has no slack right now.`,
      recommendation: severity === "critical"
        ? "Take an active rest day today. Eat at maintenance, sleep 8+ hours. Your output will improve."
        : "Add at least one rest day this week and bump calories by 150–200 kcal to support recovery.",
      trend: "declining",
      dataPoints: [
